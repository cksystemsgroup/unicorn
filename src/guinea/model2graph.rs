        ...
        ui.add_enabled_ui(!data.giraphe.leaves.is_empty(), |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button("Step until input").clicked() {
                    // TODO: what even is this monstrosity
                    while !((data
                        .giraphe
                        .nid_to_spot(&data.giraphe.registers[17].unwrap())
                        .val_cur
                        == Bitvector(Concrete(63))
                        || data
                            .giraphe
                            .nid_to_spot(&data.giraphe.registers[17].unwrap())
                            .val_cur
                        ...
                        && data
                            ...
                            .states
                            .iter()
                            .map(|x| data.giraphe.nid_to_spot(x))
                            .filter(|x| match &*x.origin.borrow() {
                                Node::State { name, .. } => {
                                    name.as_ref().unwrap().starts_with("kernel")
                                }
                                _ => unreachable!(),
                            })
            ...
            ui.add_space(10.0);
            ui.horizontal(|ui| {
                let edit = egui::TextEdit::singleline(&mut data.giraphe.input)
                    .hint_text("Value To Be Read");
                ui.add(edit);
                ui.checkbox(&mut data.giraphe.is_ascii, "Interpret as ASCII");
                if ui.button("Submit").clicked() {
                    data.giraphe.input_queue.push(format!(
                        "{} ({})",
                        data.giraphe.input.clone(),
                        if data.giraphe.is_ascii {
                            "ascii"
                        } else {
                            "number"
                        }
                    ...
                    let str: u64 = if data.giraphe.is_ascii {
                        let mut result = 0;
                        for (i, c) in data.giraphe.input.chars().enumerate() {
                            result |= (c as u64) << (i * 8);
                        }
                        result
                    } else {
                        data.giraphe.input.parse().unwrap()
                    };

                    for i in 0..data.giraphe.inputs.len() {
                        let ip = data.giraphe.inputs.get(i).unwrap();
                        let ip = data.giraphe.spot_lookup.get_mut(ip).unwrap();
                    ...
                    data.giraphe.input = String::default();
                }
            });
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    for (i, s) in zip(1.., &data.giraphe.input_queue) {

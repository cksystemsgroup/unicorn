    ...
    loop {
        for n in prev_depth..depth {
            ...
            if has_concrete_inputs {
                ...
                optimize_model_with_input(model, &mut vec![]);
            }
            //replace_memory(model);
        }
        ...
        //println!("\nDepth: {}", depth);
        //count_nodes(&good_states_initial[0]);

        if (n >= start) && !lb_found {
            /*for i in 0..67 {
                println!("\nDepth: {}", depth - i);
                count_nodes(&good_states_initial[i]);
            }*/
            ...
            if solution == SMTSolution::Sat {
                ...
                //count_nodes(&good_states_initial[n - lower_bound]);
                /*for i in 1..17 {
                    println!("\nDepth: {}", lower_bound + i);
                    count_nodes(&good_states_initial[n - lower_bound - i]);
                }*/
        ...
        if lb_found {
            /*if depth == 256 {
                count_nodes(&good_states_initial[0]);
                println!("\nDepth: {}", n - 79);
                count_nodes(&good_states_initial[79]);
            }*/
            smt_solver = S::new(timeout);
            ...
            if solution == SMTSolution::Unsat {
                ...
                //count_nodes(&good_states_initial[n - upper_bound]);
                break;
            }
            else if solution == SMTSolution::Timeout {
                println!("Timeout! [n={}]", depth);
...
fn optimize_and_get_good_states<S: SMTSolver>(
    ...
    if prune {
        ...
        optimize_model_with_solver::<S>(model, smt_solver, timeout, true, false, one_query, false);
    }
...
pub fn reason(
    ...
    match smt_solver {
        #[rustfmt::skip]
        SmtType::Generic => {
            optimize_model_with_solver::<none_impl::NoneSolver>(model, &mut NoneSolver::new(None), timeout, minimize, terminate_on_bad, one_query, stride)
        },
        #[rustfmt::skip]
        #[cfg(feature = "boolector")]
        SmtType::Boolector => {
            optimize_model_with_solver::<boolector_impl::BoolectorSolver>(model, &mut BoolectorSolver::new(timeout), timeout, minimize, terminate_on_bad, one_query, stride)
        },
        #[rustfmt::skip]
        #[cfg(feature = "z3")]
        SmtType::Z3 => {
            optimize_model_with_solver::<z3solver_impl::Z3SolverWrapper>(model, &mut Z3SolverWrapper::new(timeout), timeout, minimize, terminate_on_bad, one_query, stride)
        },
    }

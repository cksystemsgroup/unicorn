...
fn build_node(
    ...
    fn add_rtype(instruction: Instruction, rtype: RType, graph: &mut Formula, state: &mut State) {
        let node = Node::new(instruction);

        state.regs[rtype.rs1() as usize] = node.alias;
        state.regs[rtype.rs2() as usize] = node.alias;
        let node_index = graph.add_node(node);
    ...
    match instruction {
        ...
        Instruction::Lui(utype) => add_utype(instruction, utype, graph, state),
...
mod tests {
    ...
    fn can_build_formula() {
        ...
        // let _formula = build_formula_for_exit_code(&path);
